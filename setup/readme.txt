šMule Copyright (c) 2014 e1z0 ( https://code.google.com/p/smule/ )
...
Welcome to šMule, a filesharing client based on the eDonkey2000(C) network.


Visit us at
 https://code.google.com/p/smule/
or
 the IRC chatroom, #smule on irc.bsdnet.lt:7000

INSTALLATION:
-------------

-Unzip šMule to a directory of your choice, or start the installer if you 
 downloaded the .exe installer version.
 
-You can move your "Temp" and "Incoming" folders from eDonkey (or a previous 
 version of šMule) to the new directory now, in order to continue your partial 
 downloads. If you don't want to move your folders, you can set the "temp" and 
 "incoming" path in the šMule preferences and restart it to get the same 
 result.

-Updating from an earlier version of šMule: The best way to do this is simply 
 to download the .zip file (not the installer), and unzip the new smule.exe to 
 your old šMule directory, overwriting the previous one. This works for official šmule 
...
-Enter the "Download Capacity" and "Upload Capacity" according to your 
 internet connection. All values in šMule are kiloBytes (KB), your Internet 
...
-Enter "Download Limit" and "Upload Limit"  (IMPORTANT!)
 ...
 Download Limit: leave this at 0 (should šMule become too fast and you are 
...
NOTE: 56k Modem users: šMule only accepts integral values for these settings 
...
FAQ: (for more, see https://code.google.com/p/smule/ )
----

--"Will I lose my credits when switching to a new version of šMule?"
 ...
 The best way to update is just to replace your old smule.exe with the new one 
 from the .zip download.


--"Why is šMule so slow? My brother/friend/whatever is downloading at 100K constantly"

 When you did setup šMule properly, it's all about the availibility of the 
...
http://corpo.free.fr/server.met Note that those sites are not related to the šMule project, we are not responsible for their content.


--"What is the addresses.dat file for?"

 You can enter a serverlist URL in that file. šMule will then get the 
...
--"What is the staticservers.dat file good for?"
 ...
 file via the server tab in šMule (right-click -> add to static serverlist)


--"Why do I always have a low ID (means: firewalled) ??? What can I do against that?"

 Forwards your ports through the firewall or use uPNP
...
--"What does high and low ID mean anyway?"
 ...
 the forums on how to configure your firewall/router for šMule.
...
--"What do the "QR: xxxx" numbers mean that I see when I look at my sources?"
 ...
 Obviously, a lower value is better :) If the source is an šMule client and 
...
The sourcecode of šMule morph is availible as seperate download at:
 https://code.google.com/p/smule/
 
You need Microsoft(C) Visual Studio .NET 2010 + sp1 of this compiler to compile šMule.

šMule provides all required external libraries as

Download and save their sourcecode one level above the šMule source code 
folder and compile them (check the šMule-project file to learn about the 
required foldernames, or adapt them to your needs!).


-Open the smule100.sln Visual Studio Solution

-Select "release" or "debug" build in the solution configuration manager
-Build the solution. That's it :)
-If the compile was successful, the smule.exe is either in the \Debug or 
 \Release directory
...
-IMPORTANT:
 ...
 If you make modifications to šMule and you distribute the compiled version of 
...
The šMule staff would like to thank the following (no particular order)
...
šMule Copyright (C)2002-2005 Merkur (merkur-@users.sourceforge.net)
