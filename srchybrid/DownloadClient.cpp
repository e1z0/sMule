	...
	*/
	...
	if (!bUdpPacket)
	{
		if (!bPartsNeeded)
        {
			...
			&& GetReqFileAICHHash() != NULL && *GetReqFileAICHHash() == reqfile->GetFileIdentifier().GetAICHHash())) //If we are using the šMule filerequest packets, this is taken care of in the Multipacket!
			SendHashSetRequest();
		else
			SendStartupLoadReq();
	}
...
void CUpDownClient::SendBlockRequests()
{
	...
	*/
	...
	// MORPH END
        ...
        // Only trust šMule clients to be able to handle less blocks than three
	//MORPH START
	/*
...
void CUpDownClient::ProcessEmuleQueueRank(const uchar* packet, UINT size)
{
	...
	if (thePrefs.GetDebugClientTCPLevel() > 0)
		Debug(_T("  QR=%u\n"), rank); // no prev. QR available for šMule clients
	SetRemoteQueueFull(false);
	SetRemoteQueueRank(rank, GetDownloadState() == DS_ONQUEUE);
	CheckQueueRankFlood();
}
...
void CUpDownClient::ProcessAICHFileHash(CSafeMemFile* data, CPartFile* file, const CAICHHash* pAICHHash)
{
	...
	if(pPartFile != NULL && pPartFile == GetRequestFile()){
		...
		if (pPartFile->GetFileIdentifierC().HasAICHHash() && pPartFile->GetFileIdentifierC().GetAICHHash() != ahMasterHash)
		{
			...
			{
				case DS_REQHASHSET:
					// for the love of šMule, don't accept a hashset from him :)
					if (m_fHashsetRequestingMD4)
					{
