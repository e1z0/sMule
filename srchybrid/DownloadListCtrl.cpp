		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			//Commander - Added: MassRename [Dragon] - End
				...
				*/
				...
				{
					CString feed;
					feed.AppendFormat(GetResString(IDS_FEEDBACK_FROM), thePrefs.GetUserNick());
					feed.Append(_T(" \r\n"));
					POSITION pos = selectedList.GetHeadPosition();
					while (pos != NULL)
					{			
				...
				{
					CString feed;
					feed.AppendFormat(_T("Feedback from %s\r\n"),thePrefs.GetUserNick());
					POSITION pos = selectedList.GetHeadPosition();
					while (pos != NULL)
					{
						CKnownFile* file = selectedList.GetNext(pos);
...
int CDownloadListCtrl::Compare(const CUpDownClient *client1, const CUpDownClient *client2, LPARAM lParamSort, const CPartFile* pfile)
{
	...
	switch (lParamSort)
	{
		...
		case 6:
			...
			return -(client1->GetClientSoft() - client2->GetClientSoft()); // invert result to place šMule's at top
			*/
			if (client1->GetClientSoft() == client2->GetClientSoft())
			{
