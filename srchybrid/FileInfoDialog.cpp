...
public:
	...
	BOOL Initialize()
	{
		if (!m_bInitialized)
		{
			...
			// morph use prefs
			/*
			CString strPath = theApp.GetProfileString(_T("sMule"), _T("MediaInfo_MediaInfoDllPath"), _T("MEDIAINFO.DLL"));
			*/
			...
			if (m_hLib != NULL)
			{
				...
				// ---
				// šMule currently handles v0.5.1.0, v0.6.0.0, v0.6.1.0
...
bool CGetMediaInfoThread::GetMediaInfo(HWND hWndOwner, const CShareableFile* pFile, SMediaInfo* mi, bool bSingleFile)
{
	...
	if (theApp.GetProfileInt(_T("sMule"), _T("MediaInfo_RIFF"), 1))
	*/
	...
	bool bIsRM = false;
	/* morph use advanced pref
	if (theApp.GetProfileInt(_T("sMule"), _T("MediaInfo_RM"), 1))
	*/
	...
	bool bIsWM = false;
	/* morph use advanced pref
	if (theApp.GetProfileInt(_T("sMule"), _T("MediaInfo_WM"), 1))
	*/
	...
	//
	/* morph use advanced pref
	if (theApp.GetProfileInt(_T("sMule"), _T("MediaInfo_ID3LIB"), 1) &&
	*/
...
#ifdef HAVE_QEDIT_H
		/* morph use advanced pref
		if (theApp.GetProfileInt(_T("sMule"), _T("MediaInfo_MediaDet"), 1)
		*/
