		...
		if (sCommand == _T("PRIVMSG"))
		{
			...
			if (sRawMessage.Mid(iIndex, 2) == _T(":\001"))
			{
				...
				if( sRawMessage.Mid(iIndex, 9) == _T(":\001VERSION") )
				{
					...
					//Get client version.
					iIndex += 9;
					m_sVersion = _T("sMule") + theApp.m_strCurVersionLong + Irc_Version;
					CString sBuild;
				...
				if( sRawMessage.Mid(iIndex, 11) == _T(":\001RQSFRIEND") )
				{
					...
					//šMule user requested to add you as friend.
	...
	try
	{
		if (thePrefs.GetIRCJoinHelpChannel())
		{
			...
			*/
			...
			sJoinHelpChannel.Format(_T("%s|/join #sMule"),GetResString(IDS_IRC_HELPCHANNELPERFORM));
...
void CIrcMain::Connect()
{
	try
	{
		...
		if (m_sNick.CompareNoCase(_T("smule")) == 0)
			m_sNick.Format(_T("sMule IRC %u-%s"), thePrefs.GetLanguageID(), sHash);
