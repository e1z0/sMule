 ...
 // MORPH END
			...
			switch(opcode)
			{
				case OP_HELLOANSWER:
				{
					...
					//  - we have received OP_EMULEINFO and OP_HELLOANSWER (old šMule)
					//	- we have received šMule-OP_HELLOANSWER (new šMule)
				...
				case OP_HELLO:
				{
					...
					{
						// start secure identification, if
						//	- we have received šMule-OP_HELLO (new šMule)
						if (client->GetInfoPacketsReceived() == IP_BOTH)
							client->InfoPacketsReceived();
...
bool CClientReqSocket::ProcessExtPacket(const BYTE* packet, uint32 size, UINT opcode, UINT uRawSize)
{
		...
		try
		{
			...
			switch(opcode)
			{
				...
				case OP_EMULEINFO:
				{
					...
					// start secure identification, if
					//  - we have received eD2K and šMule info (old šMule)
					if (client->GetInfoPacketsReceived() == IP_BOTH)
						client->InfoPacketsReceived();
				...
				default:
					theStats.AddDownDataOverheadOther(uRawSize);
					PacketToDebugLogLine(_T("šMule"), packet, size, opcode);
					break;
			}
		}
		catch(CFileException* error)
	...
	{
		if (thePrefs.GetVerbose() && !ex->m_strMsg.IsEmpty())
			DebugLogWarning(_T("Error: %s - while processing šMule packet: opcode=%s  size=%u; %s"), ex->m_strMsg, DbgGetMuleClientTCPOpcode(opcode), size, DbgGetClientInfo());
		if (client && ex->m_bDelete)
			client->SetDownloadState(DS_ERROR, _T("Error while processing šMule packet: ") + ex->m_strMsg);
		Disconnect(ex->m_strMsg);
		ex->Delete();
		return false;
	}
	...
	{
		if (thePrefs.GetVerbose() && !error.IsEmpty())
			DebugLogWarning(_T("Error: %s - while processing šMule packet: opcode=%s  size=%u; %s"), error, DbgGetMuleClientTCPOpcode(opcode), size, DbgGetClientInfo());
		if (client)
			...
			if (thePrefs.GetConditionalTCPAccept() && !thePrefs.GetProxySettings().UseProxy)
			{
				...
				if (sNew == INVALID_SOCKET){
				    ...
				    if (nFataErrors > 10){
					    ...
					    // and lock everything. We can also just endlos try to repeat it because this will lock up šMule
