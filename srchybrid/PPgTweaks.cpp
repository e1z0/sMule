...
BOOL CPPgTweaks::OnApply()
{
	...
	if(thePrefs.m_bDateFileNameLog != (m_bDateFileNameLog != 0))
	{
		...
		VERIFY( theLog.SetFilePath(thePrefs.GetMuleDirectory(EMULE_LOGDIR) + _T("sMule.log")) );
		VERIFY( theVerboseLog.SetFilePath(thePrefs.GetMuleDirectory(EMULE_LOGDIR) + _T("sMule_Verbose.log")) );
		// [end] Mighty Knife
