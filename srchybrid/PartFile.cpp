...
bool CPartFile::HashSinglePart(UINT partnumber, bool* pbAICHReportedOK)
{
	...
	// This is the most secure way in order to make sure Å¡Mule will never deliver a corrupt file,
	// even if one of the hashalgorithms is completely or both somewhat broken
	// This however doesn't means that Å¡Mule is guaranteed to be able to finish a file in case
...
void CPartFile::SetDownPriority(uint8 np, bool resort)
{
	...
	if ( m_iDownPriority != np )
	{
		...
		UpdateDisplayedInfo(true);
		//Save the partfile. We do this so that if we restart Å¡Mule before this files does
...
bool CPartFile::IsReadyForPreview() const
{
	...
	else
	{
		...
		if (thePrefs.GetPreviewSmallBlocks() || !_tcsicmp(szVideoPlayerFileName, _T("vlc")))
		{
			...
			// but because this could disable the preview command for some file types which Å¡Mule does not know,
...
void CPartFile::GetFilledList(CTypedPtrList<CPtrList, Gap_Struct*> *filled) const
{
	...
	while (!finished)
	{
		...
		// TODO: here we have a problem - it occured that Å¡Mule crashed because of "best==NULL" while
