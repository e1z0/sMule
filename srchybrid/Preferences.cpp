...
void CPreferences::Init()
{
	...
	//
	CFileFind ff;
	BOOL bFoundFile = ff.FindFile(GetMuleDirectory(EMULE_EXECUTEABLEDIR) + L"sMule*.log", 0);
	while (bFoundFile)
	{
...
void CPreferences::SavePreferences()
{
	...
	CIni ini(GetConfigFile(), L"sMule");
	//---
	...
	*/
	...
	if (theApp.GetProfileInt(L"sMule", L"MediaInfo_RIFF_FIX", 1)==1){ // fix morph 9.3 bad default once.
	...
	//MORPH START - Added by SiRoB, [MoNKi: -UPnPNAT Support-]
	ini.WriteBool(L"UPnPNAT", m_bUPnPNat, L"sMule");
	...
	//MORPH START - Added by SiRoB, [MoNKi: -Random Ports-]
	ini.WriteBool(L"RandomPorts", m_bRndPorts, L"sMule");
	ini.WriteInt(L"MinRandomPort", m_iMinRndPort, L"sMule");
	ini.WriteInt(L"MaxRandomPort", m_iMaxRndPort, L"sMule");
	ini.WriteBool(L"RandomPortsReset", m_bRndPortsResetOnRestart, L"sMule");
	ini.WriteInt(L"RandomPortsSafeResetOnRestartTime", m_iRndPortsSafeResetOnRestartTime, L"sMule");

	ini.WriteInt(L"OldTCPRandomPort", m_iCurrentTCPRndPort, L"sMule");
	ini.WriteInt(L"OldUDPRandomPort", m_iCurrentUDPRndPort, L"sMule");
	ini.WriteUInt64(L"RandomPortsLastRun", CTime::GetCurrentTime().GetTime() , L"sMule");
	//MORPH END   - Added by SiRoB, [MoNKi: -Random Ports-]

	//MORPH START - Added by SiRoB, [MoNKi: -Improved ICS-Firewall support-]
	ini.WriteBool(L"ICFSupportFirstTime", m_bICFSupportFirstTime, L"sMule");
	ini.WriteBool(L"ICFSupport", m_bICFSupport, L"sMule");
	ini.WriteBool(L"ICFSupportServerUDP", m_bICFSupportServerUDP , L"sMule");
	//MORPH END   - Added by SiRoB, [MoNKi: -Improved ICS-Firewall support-]
	...
	//MORPH START - Added by Commander, ClientQueueProgressBar  
	ini.WriteBool(L"ClientQueueProgressBar",m_bClientQueueProgressBar, L"sMule");
	//MORPH END - Added by Commander, ClientQueueProgressBar

	//MORPH START - Added by Commander, FolderIcons  
	ini.WriteBool(L"ShowFolderIcons",m_bShowFolderIcons, L"sMule");
	//MORPH END - Added by Commander, FolderIcons

	ini.WriteBool(L"InfiniteQueue",infiniteQueue,L"sMule");	//Morph - added by AndCycle, SLUGFILLER: infiniteQueue

	ini.WriteBool(L"AutoDynUpSwitching",isautodynupswitching,L"sMule");//MORPH - Added by Yun.SF3, Auto DynUp changing
	ini.WriteInt(L"PowershareMode",m_iPowershareMode,L"sMule"); //MORPH - Added by SiRoB, Avoid misusing of powersharing
	ini.WriteBool(L"PowershareInternalPrio",m_bPowershareInternalPrio,L"sMule");//Morph - added by AndCyle, selective PS internal Prio

	ini.WriteBool(L"EnableHighProcess", enableHighProcess,L"sMule"); //MORPH - Added by IceCream, high process priority

	ini.WriteBool(L"EnableDownloadInRed", enableDownloadInRed,L"sMule"); //MORPH - Added by IceCream, show download in red
	ini.WriteBool(L"EnableDownloadInBold", m_bShowActiveDownloadsBold,L"sMule"); //MORPH - Added by SiRoB, show download in Bold
        //MORPH START - Added by schnulli900, filter clients with failed downloads [Xman]
	ini.WriteBool(L"FilterClientFailedDown", m_bFilterClientFailedDown,L"sMule"); 
        //MORPH END   - Added by schnulli900, filter clients with failed downloads [Xman]
	ini.WriteBool(L"EnableAntiLeecher", enableAntiLeecher,L"sMule"); //MORPH - Added by IceCream, enable AntiLeecher
	ini.WriteBool(L"EnableAntiCreditHack", enableAntiCreditHack,L"sMule"); //MORPH - Added by IceCream, enable AntiCreditHack
	ini.WriteInt(L"CreditSystemMode", creditSystemMode,L"sMule");// EastShare - Added by linekin, ES CreditSystem
	ini.WriteBool(L"EqualChanceForEachFile", m_bEnableEqualChanceForEachFile, L"sMule");	//Morph - added by AndCycle, Equal Chance For Each File
	ini.WriteBool(L"FollowTheMajority", m_bFollowTheMajority, L"sMule"); // EastShare       - FollowTheMajority by AndCycle
	ini.WriteInt(L"FairPlay", m_iFairPlay, L"sMule"); //EastShare	- FairPlay by AndCycle

	//MORPH START - Added by SiRoB, Datarate Average Time Management
	ini.WriteInt(L"DownloadDataRateAverageTime",max(1,m_iDownloadDataRateAverageTime/1000),L"sMule");
	ini.WriteInt(L"UPloadDataRateAverageTime",max(1,m_iUploadDataRateAverageTime/1000),L"sMule");
	//MORPH END   - Added by SiRoB, Datarate Average Time Management

	//MORPH START - Added by SiRoB, Upload Splitting Class
	ini.WriteInt(L"GlobalDataRateFriend",globaldataratefriend,L"sMule");
	ini.WriteInt(L"MaxGlobalDataRateFriend",maxglobaldataratefriend,L"sMule");
	ini.WriteInt(L"GlobalDataRatePowerShare",globaldataratepowershare,L"sMule");
	ini.WriteInt(L"MaxGlobalDataRatePowerShare",maxglobaldataratepowershare,L"sMule");
	ini.WriteInt(L"MaxClientDataRateFriend",maxclientdataratefriend,L"sMule");
	ini.WriteInt(L"MaxClientDataRatePowerShare",maxclientdataratepowershare,L"sMule");
	ini.WriteInt(L"MaxClientDataRate",maxclientdatarate,L"sMule");
	//MORPH END   - Added by SiRoB, Upload Splitting Class

	//MORPH START - Added by SiRoB, SLUGFILLER: lowIdRetry
	ini.WriteInt(L"ReconnectOnLowIdRetries",LowIdRetries,L"sMule");	// SLUGFILLER: lowIdRetry
	//MORPH END   - Added by SiRoB, SLUGFILLER: lowIdRetry
	//MORPH	Start	- Added by AndCycle, SLUGFILLER: Spreadbars - per file
	ini.WriteInt(L"SpreadbarSetStatus", m_iSpreadbarSetStatus, L"sMule");
	//MORPH	End	- Added by AndCycle, SLUGFILLER: Spreadbars - per file
	//MORPH START - Added by SiRoB, SLUGFILLER: hideOS
	ini.WriteInt(L"HideOvershares",hideOS,L"sMule");
	ini.WriteBool(L"SelectiveShare",selectiveShare,L"sMule");
	//MORPH END   - Added by SiRoB, SLUGFILLER: hideOS
	//MORPH START - Added by SiRoB, SHARE_ONLY_THE_NEED
	ini.WriteInt(L"ShareOnlyTheNeed",ShareOnlyTheNeed,L"sMule");
	//MORPH END   - Added by SiRoB, SHARE_ONLY_THE_NEED
	//MORPH START - Added by SiRoB, POWERSHARE Limit
	ini.WriteInt(L"PowerShareLimit",PowerShareLimit,L"sMule");
	//MORPH END   - Added by SiRoB, POWERSHARE Limit
	//MORPH START - Added by SiRoB, Show Permissions
	ini.WriteInt(L"ShowSharePermissions",permissions,L"sMule");
	//MORPH END   - Added by SiRoB, Show Permissions

    //MORPH START added by Yun.SF3: Ipfilter.dat update
	ini.WriteBinary(L"IPfilterVersion", (LPBYTE)&m_IPfilterVersion, sizeof(m_IPfilterVersion),L"sMule"); 
	ini.WriteBool(L"AutoUPdateIPFilter",AutoUpdateIPFilter,L"sMule");
	ini.WriteInt(L"IPFilterVersionNum", m_uIPFilterVersionNum,L"sMule");
    //MORPH END added by Yun.SF3: Ipfilter.dat update

	//Commander - Added: IP2Country Auto-updating - Start
	ini.WriteBinary(L"IP2CountryVersion", (LPBYTE)&m_IP2CountryVersion, sizeof(m_IP2CountryVersion),L"sMule"); 
	ini.WriteBool(L"AutoUPdateIP2Country",AutoUpdateIP2Country,L"sMule");
	//Commander - Added: IP2Country Auto-updating - End

	//MORPH START - Added by milobac, FakeCheck, FakeReport, Auto-updating
	ini.WriteBinary(L"FakesDatVersion", (LPBYTE)&m_FakesDatVersion, sizeof(m_FakesDatVersion),L"sMule"); 
	ini.WriteBool(L"UpdateFakeStartup",UpdateFakeStartup,L"sMule");
	//MORPH END - Added by milobac, FakeCheck, FakeReport, Auto-updating

	ini.WriteString(L"UpdateURLFakeList",UpdateURLFakeList,L"sMule");		//MORPH START - Added by milobac and Yun.SF3, FakeCheck, FakeReport, Auto-updating
	ini.WriteString(L"UpdateURLIPFilter",UpdateURLIPFilter,L"sMule");//MORPH START added by Yun.SF3: Ipfilter.dat update
    ini.WriteString(L"UpdateURLIP2Country",UpdateURLIP2Country,L"sMule");//Commander - Added: IP2Country auto-updating

	//EastShare Start - PreferShareAll by AndCycle
	ini.WriteBool(L"ShareAll",shareall,L"sMule");	// SLUGFILLER: preferShareAll
	//EastShare END - PreferShareAll by AndCycle
	// EastShare START - Added by TAHO, .met file control
	ini.WriteInt(L"KnownMetDays", m_iKnownMetDays,L"sMule");
	ini.WriteBool(L"PartiallyPurgeOldKnownFiles",m_bPartiallyPurgeOldKnownFiles,L"sMule");
	ini.WriteBool(L"CompletlyPurgeOldKnownFiles",m_bCompletlyPurgeOldKnownFiles,L"sMule");
	ini.WriteBool(L"RemoveAichImmediatly",m_bRemoveAichImmediatly,L"sMule");
	// EastShare END - Added by TAHO, .met file control
	//EastShare - Added by Pretender, Option for ChunkDots
	ini.WriteInt(L"EnableChunkDots", m_bEnableChunkDots,L"sMule");
	//EastShare - Added by Pretender, Option for ChunkDots

	//EastShare - added by AndCycle, IP to Country
	ini.WriteInt(L"IP2Country", m_iIP2CountryNameMode,L"sMule"); 
	ini.WriteBool(L"IP2CountryShowFlag", m_bIP2CountryShowFlag,L"sMule");
	//EastShare - added by AndCycle, IP to Country

	// khaos::categorymod+ Save Preferences
	ini.WriteBool(L"ValidSrcsOnly", m_bValidSrcsOnly,L"sMule");
	ini.WriteBool(L"ShowCatName", m_bShowCatNames,L"sMule");
	ini.WriteBool(L"ActiveCatDefault", m_bActiveCatDefault,L"sMule");
	ini.WriteBool(L"SelCatOnAdd", m_bSelCatOnAdd,L"sMule");
	ini.WriteBool(L"AutoSetResumeOrder", m_bAutoSetResumeOrder,L"sMule");
	ini.WriteBool(L"SmallFileDLPush", m_bSmallFileDLPush,L"sMule");
	ini.WriteInt(L"StartDLInEmptyCats", m_iStartDLInEmptyCats,L"sMule");
	ini.WriteBool(L"UseAutoCat", m_bUseAutoCat,L"sMule");
	ini.WriteBool(L"AddRemovedInc", m_bAddRemovedInc,L"sMule");
	// khaos::categorymod-
	// MORPH START leuk_he disable catcolor
	ini.WriteBool(L"DisableCatColors", m_bDisableCatColors,L"sMule");
	// MORPH END   leuk_he disable catcolor
	// khaos::kmod+
	ini.WriteBool(L"SmartA4AFSwapping", m_bSmartA4AFSwapping,L"sMule");
	ini.WriteInt(L"AdvancedA4AFMode", m_iAdvancedA4AFMode,L"sMule");
	ini.WriteBool(L"ShowA4AFDebugOutput", m_bShowA4AFDebugOutput,L"sMule");
	ini.WriteBool(L"RespectMaxSources", m_bRespectMaxSources,L"sMule");
	ini.WriteBool(L"UseSaveLoadSources", m_bUseSaveLoadSources,L"sMule");
	// khaos::categorymod-
	// khaos::accuratetimerem+
	ini.WriteInt(L"TimeRemainingMode", m_iTimeRemainingMode,L"sMule");
	// khaos::accuratetimerem-
	//MORPH START - Added by SiRoB, ICS Optional
	ini.WriteBool(L"UseIntelligentChunkSelection", m_bUseIntelligentChunkSelection,L"sMule");
	//MORPH END   - Added by SiRoB, ICS Optional
	//MORPH START - Added by SiRoB, Smart Upload Control v2 (SUC) [lovelace]
	ini.WriteBool(L"SUCEnabled",m_bSUCEnabled,L"sMule");
	ini.WriteInt(L"SUCLog",m_bSUCLog,L"sMule");
	ini.WriteInt(L"SUCHigh",m_iSUCHigh,L"sMule");
	ini.WriteInt(L"SUCLow",m_iSUCLow,L"sMule");
	ini.WriteInt(L"SUCDrift",m_iSUCDrift,L"sMule");
	ini.WriteInt(L"SUCPitch",m_iSUCPitch,L"sMule");
	//MORPH END - Added by SiRoB, Smart Upload Control v2 (SUC) [lovelace]
	ini.WriteInt(L"MaxConnectionsSwitchBorder",maxconnectionsswitchborder,L"sMule");//MORPH - Added by Yun.SF3, Auto DynUp changing

	ini.WriteBool(L"IsPayBackFirst",m_bPayBackFirst,L"sMule");//EastShare - added by AndCycle, Pay Back First
	ini.WriteInt(L"PayBackFirstLimit",m_iPayBackFirstLimit,L"sMule");//MORPH - Added by SiRoB, Pay Back First Tweak
	ini.WriteBool(L"OnlyDownloadCompleteFiles", m_bOnlyDownloadCompleteFiles,L"sMule");//EastShare - Added by AndCycle, Only download complete files v2.1 (shadow)
	ini.WriteBool(L"SaveUploadQueueWaitTime", m_bSaveUploadQueueWaitTime,L"sMule");//Morph - added by AndCycle, Save Upload Queue Wait Time (MSUQWT)
	ini.WriteBool(L"DateFileNameLog", m_bDateFileNameLog,L"sMule");//Morph - added by AndCycle, Date File Name Log
	ini.WriteBool(L"DontRemoveSpareTrickleSlot", m_bDontRemoveSpareTrickleSlot,L"sMule");//Morph - added by AndCycle, Dont Remove Spare Trickle Slot
	ini.WriteBool(L"UseDownloadOverhead",m_bUseDownloadOverhead,L"sMule");//Morph - added by AndCycle, Dont Remove Spare Trickle Slot
	ini.WriteBool(L"DisplayFunnyNick", m_bFunnyNick,L"sMule");//MORPH - Added by SiRoB, Optionnal funnynick display
	//EastShare Start - Added by Pretender, TBH-AutoBackup
	ini.WriteBool(L"AutoBackup",autobackup,L"sMule");
	ini.WriteBool(L"AutoBackup2",autobackup2,L"sMule");
	//EastShare End - Added by Pretender, TBH-AutoBackup

	// Mighty Knife: Community visualization, Report hashing files, Log friendlist activities
	ini.WriteString(L"CommunityName", m_sCommunityName,L"sMule");
	ini.WriteBool (L"ReportHashingFiles",m_bReportHashingFiles,L"sMule");
	ini.WriteBool (L"LogFriendlistActivities",m_bLogFriendlistActivities,L"sMule");
	// [end] Mighty Knife

	// Mighty Knife: CRC32-Tag
	ini.WriteBool (L"DontAddCRC32ToFilename",m_bDontAddCRCToFilename,L"sMule");
	ini.WriteBool (L"ForceCRC32Uppercase",m_bCRC32ForceUppercase,L"sMule");
	ini.WriteBool (L"ForceCRC32Adding",m_bCRC32ForceAdding,L"sMule");
	CString temp;
	...
	ini.WriteString(L"LastCRC32Prefix",temp,L"sMule");
	temp.Format (L"\"%s\"",m_sCRC32Suffix);
	ini.WriteString(L"LastCRC32Suffix",temp,L"sMule");
	// [end] Mighty Knife
	...
	// Mighty Knife: Static server handling
	ini.WriteBool (L"DontRemoveStaticServers",m_bDontRemoveStaticServers,L"sMule");
	// [end] Mighty Knife

	//MORPH START - Added by SiRoB,  ZZ dynamic upload (USS)
	ini.WriteBool(L"USSLog", m_bDynUpLog,L"sMule");
	//MORPH END    - Added by SiRoB,  ZZ dynamic upload (USS)
	ini.WriteBool(L"USSUDP_FORCE", m_bUSSUDP,L"sMule"); //MORPH - Added by SiRoB, USS UDP preferency
	...
	if(theApp.GetProfileInt(_T("sMule"), _T("ServiceOptLvlNew"),-1) == 4 && m_iServiceOptLvl == SVC_BASIC_OPT) // Import from future StulleMule versions
...
void CPreferences::LoadPreferences()
{
	...
	*/
	...
	if (maxGraphUploadRate == 0)
		...
		if (nOldUploadCapacity == 16 && ini.GetInt(L"MaxUpload",12) == 12){
			...
			maxGraphUploadRate = UNLIMITED;
			ini.WriteInt(L"MaxUpload",UNLIMITED, L"sMule");
		}
		else
  ...
  // MORPH END leuk_he upnp bindaddr
	...
	m_iCompressLevel=(short) ini.GetInt(L"CompressLevel",9,L"sMule" );
...
//	resumeSameCat=ini.GetBool(L"ResumeNextFromSameCat",false);
	...
	*/
	...
	m_strTemplateFile = ini.GetString(L"WebTemplateFile", GetMuleDirectory(EMULE_EXECUTEABLEDIR) + L"sMule.tmpl");
	...
	if (m_strTemplateFile.Compare(GetMuleDirectory(EMULE_EXECUTEABLEDIR) + L"sMule.tmpl") == 0){
		CFileFind ff;
		if (ff.FindFile(GetMuleDirectory(EMULE_CONFIGDIR) + L"sMule.tmpl"))
			m_strTemplateFile = GetMuleDirectory(EMULE_CONFIGDIR) + L"sMule.tmpl";
		ff.Close();
	}
  ...
  m_bCryptLayerRequiredStrictServer = ini.GetBool(L"CryptLayerRequiredStrictServer",false); // MORPH lh require obfuscated server connection 
    ...
    //MORPH START - Added by SiRoB, [MoNKi: -UPnPNAT Support-]
	m_bUPnPNat = ini.GetBool(L"UPnPNAT", false, L"sMule");
	m_bUPnPNatWeb = ini.GetBool(L"UPnPNAT_Web", false, L"sMule");
	m_bUPnPVerboseLog = ini.GetBool(L"UPnPVerbose", true, L"sMule");
	m_iUPnPPort = (uint16)ini.GetInt(L"UPnPPort", 0, L"sMule");
	m_bUPnPLimitToFirstConnection = ini.GetBool(L"UPnPLimitToFirstConnection", false, L"sMule");
	m_bUPnPClearOnClose = ini.GetBool(L"UPnPClearOnClose", true, L"sMule");
    SetUpnpDetect(ini.GetInt(L"uPnPDetect", UPNP_DO_AUTODETECT, L"sMule")); //leuk_he autodetect upnp in wizard
    m_bUPnPForceUpdate=ini.GetBool(L"UPnPForceUpdate", false);
	//MORPH END   - Added by SiRoB, [MoNKi: -UPnPNAT Support-]

	//MORPH START - Added by SiRoB, [MoNKi: -Random Ports-]
	m_bRndPorts = ini.GetBool(L"RandomPorts", false, L"sMule");
	m_iMinRndPort = (uint16)ini.GetInt(L"MinRandomPort", 3000, L"sMule");
	m_iMaxRndPort = (uint16)ini.GetInt(L"MaxRandomPort", 0xFFFF, L"sMule");
	m_bRndPortsResetOnRestart = ini.GetBool(L"RandomPortsReset", false, L"sMule");
	m_iRndPortsSafeResetOnRestartTime = (uint16)ini.GetInt(L"RandomPortsSafeResetOnRestartTime", 0, L"sMule");
	
	uint16 iOldRndTCPPort = (uint16)ini.GetInt(L"OldTCPRandomPort", 0, L"sMule");
	uint16 iOldRndUDPPort = (uint16)ini.GetInt(L"OldUDPRandomPort", 0, L"sMule");
	__time64_t iRndPortsLastRun = ini.GetUInt64(L"RandomPortsLastRun", 0, L"sMule");
	...
	m_bICFSupport = ini.GetBool(L"ICFSupport", false, L"sMule");
	m_bICFSupportFirstTime = ini.GetBool(L"ICFSupportFirstTime", true, L"sMule");
	m_bICFSupportServerUDP = ini.GetBool(L"ICFSupportServerUDP", false, L"sMule");
	...
	_stprintf(m_sWapTemplateFile,L"%s",ini.GetString(L"WapTemplateFile",L"sMule_Wap.tmpl",L"WapServer"));
	...
	//MORPH START - Added by Stulle, Global Source Limit
	m_bGlobalHL = ini.GetBool(L"GlobalHL", true, L"sMule");
	...
	//MORPH START leuk_he Advanced official preferences.
	bMiniMuleAutoClose=ini.GetBool(L"MiniMuleAutoClose",0,L"sMule");
...
bool CPreferences::IsInstallationDirectory(const CString& rstrDir)
{
	...
	// skip sharing of several special šMule folders
...
bool CPreferences::IsShareableDirectory(const CString& rstrDir)
{
	...
	for (int i=0;i<GetTempDirCount();i++)
		if (!CompareDirectories(strFullPath, GetTempDir(i)))			// ".\sMule\temp"
			return false;
...
CString CPreferences::GetHomepageBaseURLForLevel(int nLevel){
	CString tmp;
	if (nLevel == 0)
		tmp = L"https://code.google.com/p/smule/";
	else if (nLevel == 1)
		tmp = L"https://code.google.com/p/smule/";
	else if (nLevel == 2)
		tmp = L"https://code.google.com/p/smule/";
	else if (nLevel < 100)
		tmp.Format(L"https://code.google.com/p/smule/");
	else if (nLevel < 150)
		tmp.Format(L"https://code.google.com/p/smule/");
	else if (nLevel < 200)
		tmp.Format(L"https://code.google.com/p/smule/");
...
CString CPreferences::GetVersionCheckBaseURL(){
	...
	if (nWebMirrorAlertLevel < 100)
		...
		tmp.Format(L"https://code.google.com/p/smule",nWebMirrorAlertLevel);
...
bool CPreferences::IsDefaultNick(const CString strCheck){
	...
	return ( strCheck == L"https://code.google.com/p/smule" );
}
...
CString CPreferences::GetDefaultDirectory(EDefaultDirectory eDirectory, bool bCreate){
	...
	if (m_astrDefaultDirs[0].IsEmpty()){ // already have all directories fetched and stored?	
		...
		*pszFileName = L'\0';
		...
		if (rkEMuleRegKey.Open(HKEY_CURRENT_USER, _T("Software\\sMule"), KEY_READ) == ERROR_SUCCESS){
						...
						&& (*pfnSHGetKnownFolderPath)(FOLDERID_ProgramData, 0, NULL, &pszProgrammData) == S_OK)
					{
							...
							&& _tcsclen(pszProgrammData) < MAX_PATH - 30 && _tcsclen(pszPublicDownloads) < MAX_PATH - 40)
						{
							...
							if (nRegistrySetting == -1){
								...
								bool bRes =  ff.FindFile(strLocalAppData + _T("sMule\\") + CONFIGFOLDER + _T("preferences.ini"), 0) != 0;
								ff.Close();
								if (bRes)
									m_nCurrentUserDirMode = 0;
								else{
									bRes =  ff.FindFile(strProgrammData + _T("sMule\\") + CONFIGFOLDER + _T("preferences.ini"), 0) != 0;
									ff.Close();
									if (bRes)
							...
							if (m_nCurrentUserDirMode == 0){
								// multiuser
								strSelectedDataBaseDirectory = strPersonalDownloads + _T("sMule\\");
								strSelectedConfigBaseDirectory = strLocalAppData + _T("sMule\\");
								strSelectedExpansionBaseDirectory = strProgrammData + _T("sMule\\");
							}
							else if (m_nCurrentUserDirMode == 1){
								// public user
								strSelectedDataBaseDirectory = strPublicDownloads + _T("sMule\\");
								strSelectedConfigBaseDirectory = strProgrammData + _T("sMule\\");
								strSelectedExpansionBaseDirectory = strProgrammData + _T("sMule\\");
							}
					...
					if (!strAppData.IsEmpty() && !strPersonal.IsEmpty())
					{
						if (strAppData.GetLength() < MAX_PATH - 30 && strPersonal.GetLength() < MAX_PATH - 40){
							...
							if (nRegistrySetting == 0){
								...
								strSelectedDataBaseDirectory = strPersonal + _T("sMule Downloads\\");
								strSelectedConfigBaseDirectory = strAppData + _T("sMule\\");
								m_nCurrentUserDirMode = 0;
								// strSelectedExpansionBaseDirectory stays default
							}
							else if (nRegistrySetting == -1 && !bConfigAvailableExecuteable){
								if (ff.FindFile(strAppData + _T("sMule\\") + CONFIGFOLDER + _T("preferences.ini"), 0)){
	...
	if (bMoveConfigFiles)
	{
		...
		CopyFile(strProgDirConfigPath + _T("sMule Light.tmpl"), m_astrDefaultDirs[EMULE_CONFIGDIR] + _T("sMule Light.tmpl"),TRUE);
		CopyFile(strProgDirConfigPath + _T("sMule.tmpl"), m_astrDefaultDirs[EMULE_CONFIGDIR] + _T("sMule.tmpl"),TRUE);
		CopyFile(strProgDirConfigPath + _T("Multiuser sMule.tmpl"), m_astrDefaultDirs[EMULE_CONFIGDIR] + _T("Multiuser sMule.tmpl"),TRUE);
...
void CPreferences::ChangeUserDirMode(int nNewMode){
	...
	CRegKey rkEMuleRegKey;
	if (rkEMuleRegKey.Create(HKEY_CURRENT_USER, _T("Software\\sMule")) == ERROR_SUCCESS){
...
bool CPreferences::GetSparsePartFiles()	{
	...
	// Due to the fact that šMule wirtes a lot small blocks into sparse files and flushs them every 6 seconds,
...
bool CPreferences::IsRunningAeroGlassTheme(){
	...
	// in other words, its draw all or nothing yourself - šMule chooses currently nothing
...
int CPreferences::GetServiceStartupMode(){
	...
	if (m_iServiceStartupMode == 0) // may be called before LoadPreferences()
	   m_iServiceStartupMode=theApp.GetProfileInt(_T("sMule"), _T("ServiceStartupMode"),2); // default = stop service and start
