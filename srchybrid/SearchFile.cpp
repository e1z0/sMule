						 ...
						 uint32 nSearchID, uint32 nServerIP, uint16 nServerPort, LPCTSTR pszDirectory, bool bKademlia, bool bServerUDPAnswer)
{
	...
	for (UINT i = 0; i < tagcount; i++)
	{
		...
		if (tag)
		{
			...
			// a local tag with an šMule known rating value and drop the percentage (which is currently done),
			// or add a second tag which holds the percentage in addition to the šMule-known rating value.
