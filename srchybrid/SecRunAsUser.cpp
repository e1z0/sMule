...
eResult CSecRunAsUser::PrepareUser(){
	...
	try{
		...
		try{
			...
			// check if we are already running on our šMule Account
			...
			if (m_strCurrentUser == EMULEACCOUNTW){ 
			    ...
			    throw CString(_T("Already running as šMule_Secure Account (everything is fine)"));
			}
...
bool CSecRunAsUser::CreateEmuleUser(IADsContainerPtr pUsers){
	...
	pUser->put_Description(CString(_T("Account used to run šMule with additional security")).AllocSysString() );
	pUser->SetInfo();
	m_strPassword = CreateRandomPW();
...
eResult CSecRunAsUser::RestartAsUser(){
	...
	FreeAPI();
	if (!bResult)
		theApp.QueueDebugLogLine(false, _T("Run as unpriveleged user: Error: Failed to restart šMule as different user! Error Code: %i"),GetLastError());
