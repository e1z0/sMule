...
#ifdef _BETA
	// On Beta builds we created a testfile, delete it when closing šMule
	CString tempDir = thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR);
	if (tempDir.Right(1)!=_T("\\"))
		tempDir+=_T("\\");
	CString strBetaFileName;
	strBetaFileName.Format(_T("sMule%u.%u%c.%u Beta Testfile "), CemuleApp::m_nVersionMjr, 
...
/* old code:
	...
	else
	{
		try	{
			...
			f.WriteString(_T("This file is automatically created by šMule Beta versions to help the developers testing and debugging new the new features. šMule will delete this file when exiting, otherwise you can remove this file at any time.\nThanks for beta testing šMule :)"));
			f.Close();
		}
											  ...
											  CServer* pServer, CUpDownClient* pClient)
{
	...
	// eserver 17.6+ supports šMule file rating tag. There is no TCP-capabilities bit available to determine
	...
	// eserver 16.4+ does not need the FT_FILEFORMAT tag at all nor does any šMule client. This tag
	// was used for older (very old) eDonkey servers only. -> We send it only to non-šMule clients.
		...
		for (int i = 0; i < ARRSIZE(_aMetaTags); i++)
		{
			...
			if (pTag != NULL)
			{
				...
				else if (_aMetaTags[i].nName == FT_MEDIA_LENGTH && pTag->IsInt())
				{
					...
					// All 'eserver' versions and šMule versions >= 0.42.4 support the media length tag with type 'integer'
