...
CString CStatisticsTree::GetText(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	if (bPrintHeader)
		//MORPH START - Changed by SiRoB, [itsonlyme: -modname-]
		/*
		strBuffer.Format(_T("šMule v%s %s [%s]\r\n\r\n"), theApp.m_strCurVersionLong, GetResString(IDS_SF_STATISTICS) ,thePrefs.GetUserNick());
		*/
		strBuffer.Format(_T("šMule v%s %s [%s]\r\n\r\n"), theApp.m_strCurVersionLong, GetResString(IDS_SF_STATISTICS) ,thePrefs.GetUserNick());
		//MORPH END   - Changed by SiRoB, [itsonlyme: -modname-]
...
// Styled ID Tags:	pghdr	= This is used for the header that gives the šMule build and date.
...
void CStatisticsTree::ExportHTML()
{
	...
	CFileDialog saveAsDlg(false, _T("html"), _T("sMule Statistics.html"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER, _T("HTML Files (*.html)|*.html|All Files (*.*)|*.*||"), this, 0);
	if (saveAsDlg.DoModal() == IDOK)
	{
		theApp.emuledlg->statisticswnd->ShowStatistics(true);// force update
		CString		strHTML;

		strHTML.Format( CString("<html>\r\n<header>\r\n<title>šMule %s [%s]</title>\r\n"
			...
			"<span id=\"pghdr\"><b>šMule %s</b></span><br /><span id=\"pghdr2\">%s %s</span>\r\n<br /><br />\r\n"
			"%s</body></html>") ,
...
CString CStatisticsTree::GetWML(bool onlyVisible, bool onlyBold, bool noRecursive, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	if (firstItem) strBuffer.Format(_T("<b>šMule v%s %s [%s]</b>\r\n<br/><br/>\r\n"), theApp.m_strCurVersionLong, GetResString(IDS_SF_STATISTICS), thePrefs.GetUserNick());
