...
void CWebServer::ReloadTemplates()
{
	...
	if (file.Open(sFile, CFile::modeRead|CFile::shareDenyWrite|CFile::typeText))
	{
		...
		else
		{
			...
			//MORPH START - Added by SiRoB/Commander, Login Failed from šMule+
			m_Templates.sFailedLogin = _LoadTemplate(sAll,_T("TMPL_FAILEDLOGIN"));
			//MORPH END   - Added by SiRoB/Commander, Login Failed from šMule+
...
//	Auto-refresh code
	...
	*/
	...
	Out.Replace(_T("[eMuleAppName]"), _T("šMule"));
