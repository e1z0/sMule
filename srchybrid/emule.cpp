...
void ShowUnicowsError()
{
	...
	// NOTE: Do *NOT* use šMule's localization functions here!
	MessageBoxA(NULL,
				"This šMule version requires the \"Microsoft(R) Layer for Unicode(TM) on Windows(R) 95/98/ME Systems\".\r\n"
				"\r\n"
				...
				"or\r\n"
				"        visit the šMule Project Download Page https://code.google.com/p/smule/\r\n"
				"or\r\n"
				"        search the Microsoft(R) Download Center http://www.microsoft.com/downloads/ for \"MSLU\" or \"unicows\"."
				"\r\n"
				"\r\n"
				"\r\n"
				"After downloading the MSLU package, run the \"unicows.exe\" program and specify your šMule installation folder "
				"where to place the extracted files from the package.\r\n"
				"\r\n"
				"Ensure that the file \"unicows.dll\" was placed in your šMule installation folder and start šMule again.",
				"šMule",
...
extern "C" HMODULE __stdcall ExplicitPreLoadUnicows()
{
	...
	// NOTE: Do *NOT* use šMule's localization functions here!
	MessageBoxA(NULL, 
				"This šMule version (Unicode, MSLU, shared MFC) does not run with this version of Windows.", 
				"šMule", 
				MB_ICONSTOP | MB_SYSTEMMODAL | MB_SETFOREGROUND);
	exit(1);
#endif
...
//CString _strCrtDebugReportFilePath(_T("šMule CRT Debug Log.log"));
// don't use a CString for that memory - it will not be available on application termination!
#define APP_CRT_DEBUG_LOG_FILE _T("šMule CRT Debug Log.log")
...
// object files which are used to link šMule.exe, the linker may or may not
// throw some errors about 'safeseh'. Those errors have to get resolved until
// the linker is capable of linking šMule.exe *with* "/SafeSEH".
//
...
void InitSafeSEH()
{
	...
	{
		AfxMessageBox(_T("sMule.exe was not linked with /SafeSEH!"), MB_ICONSTOP);
	}
}
...
BOOL CemuleApp::InitInstance()
{
	...
	//
#ifndef _BETA
	if (GetProfileInt(_T("sMule"), _T("CreateCrashDump"), 0))
#endif
		...
		*/
		theCrashDumper.Enable(_T("šMule ") + m_strCurVersionLongDbg, true, thePrefs.GetMuleDirectory(EMULE_CONFIGDIR));
   ...
   // leuk_he: prevent switch to ... busy popup during windows startup. see kb 248019
	...
	{
		if (GetProfileInt(_T("sMule"), _T("CheckComctl32"), 1)) // just in case some user's can not install that package and have to survive without it..
		{
			...
			// No need to exit šMule, it will most likely work as expected but it will have some GUI glitches here and there..
		}
	}
	...
	{
		if (GetProfileInt(_T("sMule"), _T("CheckShell32"), 1)) // just in case some user's can not install that package and have to survive without it..
		{
			AfxMessageBox(_T("Windows Shell library (SHELL32.DLL) is too old!\r\n\r\nšMule detected a version of the \"Windows Shell library (SHELL32.DLL)\" which is too old to be properly used by šMule. To ensure full and flawless functionality of šMule we strongly recommend to update the \"Windows Shell library (SHELL32.DLL)\" to at least version 4.7.\r\n\r\nDownload and install an update of the \"Windows Shell library (SHELL32.DLL)\" at Microsoft (R) Download Center."), MB_ICONSTOP);
...
#error "You are using an MFC version which may require a special version of the above function!"
	...
	// check if we have to restart šMule as Secure user
...
#endif
	VERIFY( theLog.SetFilePath(thePrefs.GetMuleDirectory(EMULE_LOGDIR, thePrefs.GetLog2Disk()) + _T("sMule.log")) );
	VERIFY( theVerboseLog.SetFilePath(thePrefs.GetMuleDirectory(EMULE_LOGDIR, false) + _T("sMule_Verbose.log")) );
	...
	Log(_T("Starting šMule v%s"), m_strCurVersionLong);
	...
	if(!thePrefs.GetICFSupport()&& !IsRunningXPSP2() && thePrefs.GetICFSupportFirstTime() && m_pFirewallOpener->DoesFWConnectionExist()){ 	 
		if(MessageBox(NULL, GetResString(IDS_ICFSUPPORTFIRST), _T("šMule"), MB_YESNO | MB_ICONQUESTION) == IDYES){ 	 
	...
	*/
	...
	//MORPH END   - Changed by SiRoB, [MoNKi: -Random Ports-]
		if (m_pFirewallOpener->DoesFWConnectionExist()){
			...
			/*
			// delete old rules added by šMule
...
{
	bool bIgnoreRunningInstances = (GetProfileInt(_T("sMule"), _T("IgnoreInstances"), 0) != 0);
	...
	UINT uTcpPort = GetProfileInt(_T("sMule"), _T("Port"), DEFAULT_TCP_PORT_OLD);
	...
	//this codepart is to determine special cases when we do add a link to our šMule
...
bool CemuleApp::CopyTextToClipboard(CString strText)
{
	...
	/* MORPH start moved to before setclipboarddata 
		...
		IgnoreClipboardLinks(strText); // this is so šMule won't think the clipboard has ed2k links for adding
...
{ 
	...
	//		compatibility with older šMule versions.
...
bool CemuleApp::GetLangHelpFilePath(CString& strResult)
{
	...
	if (thePrefs.GetLanguageID() != MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT))
	{
		...
		CString temp;
		temp.Format(_T("%s\\sMule.%u.chm"), strHelpFile.Left(pos), thePrefs.GetLanguageID());
		if (pos>0)
			strHelpFile = temp;
		...
		CString temp;
		temp.Format(_T("%s\\sMule.chm"), strHelpFile.Left(pos));
		strHelpFile = temp;
		strHelpFile.Replace(_T(".HLP"), _T(".chm"));
		if (!ff.FindFile(strHelpFile, 0))
			bFound = false;
...
{
	CString strHelpURL;
	strHelpURL.Format(_T("https://code.google.com/p/smule/wiki/help"));
...
#ifdef _DEBUG
	if (!g_bLowColorDesktop)
		g_bLowColorDesktop = (GetProfileInt(_T("sMule"), _T("LowColorRes"), 0) != 0);
#endif
...
BOOL WINAPI ConsoleCtrlHandler(DWORD dwCtrlType)
{
	...
	//
	// *) This function is invoked only, if šMule was started with "RUNAS"
	...
	//		- when šMule issues a ExitWindowsEx(EWX_LOGOFF/EWX_REBOOT/EWX_SHUTDOWN)
	//
