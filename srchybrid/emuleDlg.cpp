...
// Allows "invisible mode" on multiple instances of šMule
#ifdef _DEBUG
#define EMULE_GUID_INVMODE				"SMULE-{4EADC6FC-516F-4b7c-9066-97D893649569}-DEBUG-INVISIBLEMODE"
#else
#define EMULE_GUID_INVMODE				"SMULE-{4EADC6FC-516F-4b7c-9066-97D893649569}-INVISIBLEMODE"
#endif
	...
	*/
	...
	if (m_bStartMinimized)
	{
		...
		if (*thePrefs.GetMinTrayPTR() || thePrefs.GetInvisibleMode() && theApp.DidWeAutoStart())//MORPH - Changed by SiRoB, Invisible Mode
		{
			...
			//
			// Unfortunately this does not work. The šMule main window is a modal dialog which is invoked
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	*/
	...
	if (IsWindowVisible() && thePrefs.ShowRatesOnTitle())
	{
		...
		szBuff[_countof(szBuff) - 1] = _T('\0');
		*/
		_sntprintf(szBuff, _countof(szBuff), _T("(U:%.1f D:%.1f) šMule v%s"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong);
		szBuff[_countof(szBuff) - 1] = _T('\0');
...
void CemuleDlg::OnEndSession(BOOL bEnding)
{
	...
	{
		// If šMule was *not* started with "RUNAS":
		...
		// does not cancel the Windows dialog, Windows will though wait until šMule has 
...
LRESULT CemuleDlg::OnUserChanged(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	// logoff/reboot/shutdown problem when šMule was started with "RUNAS".
	return Default();
}
...
LRESULT CemuleDlg::OnConsoleThreadEvent(WPARAM wParam, LPARAM lParam)
{
	...
	// If šMule was started with "RUNAS":
...
void CemuleDlg::OnClose()
{
	...
	AddLogLine(true,_T("Closing šMule")); // morph log to status bar. 
	CloseTTS();
...
*/
	...
	//EastShare, Added by linekin HotKey
	AddDebugLogLine(DLP_VERYLOW, _T("Closed šMule"));
...
LRESULT CemuleDlg::OnTaskbarNotifierClicked(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	switch (m_wndTaskbarNotifier->GetMessageType())
	{
		...
		{
			CString theUrl;
			theUrl.Format(_T("/wiki/naujausia_versija"),theApp.m_uCurVersionCheck,thePrefs.GetLanguageID());
		...
		case TBN_NEWMVERSION:
		{
			//ShellExecute(NULL, NULL, _T(""), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		//MORPH END   - Added by SiRoB, New Version Check
	}
...
HBRUSH CemuleDlg::GetCtlColor(CDC* /*pDC*/, CWnd* /*pWnd*/, UINT nCtlColor)
{
	...
	// This function could be used to give the entire šMule (at least all of the main windows)
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		case MP_HM_LINK3: {
			CString theUrl;
			theUrl.Format( thePrefs.GetVersionCheckBaseURL() + CString(_T("/wiki/naujausia_versija")),theApp.m_uCurVersionCheck,thePrefs.GetLanguageID());
			ShellExecute(NULL, NULL, theUrl, NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
			break;
		}
		case MP_WEBSVC_EDIT:
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				// last byte contains informations about mirror urls, to avoid effects of future DDoS Attacks against šMules Homepage
...
BOOL CemuleApp::IsIdleMessage(MSG *pMsg)
{
	...
	// in šMule (we have a rather high frequency timer in upload queue). To save CPU load but
	...
	// filter all WM_TIMER messages. If šMule is running in taskbar the only messages which
...
void CemuleDlg::CreateToolbarCmdIconMap()
{
	...
	m_mapTbarCmdToIcon.SetAt(TBBTN_IRC, _T("Chat"));
...
BOOL CemuleDlg::OnChevronPushed(UINT id, NMHDR* pNMHDR, LRESULT* plResult)
{
	...
	CTitleMenu menu;
	menu.CreatePopupMenu();
	menu.AddMenuTitle(_T("šMule"), true);
	while (i < iButtons)
	{
		TCHAR szString[256];
		szString[0] = _T('\0');
	...
	if (m_pTaskbarList && thePrefs.IsWin7TaskbarGoodiesEnabled()) 
	{
		...
		if (thePrefs.IsShowUpDownIconInTaskbar()) 
		{
			...
			if (m_ovlIcon!=newicon) {
				m_ovlIcon=newicon;
				m_pTaskbarList->SetOverlayIcon ( m_hWnd, m_ovlIcon, _T("šMule Up/Down Indicator") );
			}
		}
	}
}
...
BOOL CemuleDlg::UnRegisterInvisibleHotKey()
{
	if(m_hWnd){
		...
		// Allows "invisible mode" on multiple instances of šMule
		// Only one app (šMule) can register the hotkey, if we unregister, we need
