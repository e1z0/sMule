...
// Returns a file type which is used šMule internally only, examining the extension of the given filename
...
// Returns a file type which is used šMule internally only (GUI)
...
void InstallSkin(LPCTSTR pszSkinPackage)
{
	...
	else if (_tcscmp(szExt, _T(".rar")) == 0)
	{
		...
		if (rar.Open(pszSkinPackage))
		{
			...
			CString strError;
			strError.Format(_T("%s\r\n\r\nDownload latest version of UNRAR.DLL from http://www.rarlab.com and copy UNRAR.DLL into šMule installation folder."), GetResString(IDS_INSTALL_SKIN_PKG_ERROR));
			AfxMessageBox(strError, MB_ICONERROR);
		}
	}
}
...
bool DoCollectionRegFix(bool checkOnly)
{
	...
	// first check if the registry keys are already set (either by installer in HKLM or by user in HKCU)
	result = regkey.Open(HKEY_CLASSES_ROOT, _T("sMule\\shell\\open\\command"), KEY_READ);
	if (result == ERROR_SUCCESS)
	{
		TCHAR rbuffer[MAX_PATH + 100];
		ULONG maxsize = _countof(rbuffer);
	...
	if (!bGlobalSet)
	{
		...
		if (checkOnly)
			return true;
		HKEY hkeyCR = thePrefs.GetWindowsVersion() < _WINVER_2K_ ? HKEY_LOCAL_MACHINE : HKEY_CURRENT_USER;
		if (regkey.Create(hkeyCR, _T("Software\\Classes\\sMule\\shell\\open\\command")) == ERROR_SUCCESS)
		{
			VERIFY( regkey.SetStringValue(NULL, regbuffer) == ERROR_SUCCESS );

			VERIFY( regkey.Create(hkeyCR, _T("Software\\Classes\\sMule\\DefaultIcon")) == ERROR_SUCCESS );
			VERIFY( regkey.SetStringValue(NULL, CString(modbuffer) + _T(",1")) == ERROR_SUCCESS );

			VERIFY( regkey.Create(hkeyCR, _T("Software\\Classes\\sMule")) == ERROR_SUCCESS );
			VERIFY( regkey.SetStringValue(NULL, _T("sMule Collection File")) == ERROR_SUCCESS );

			VERIFY( regkey.Open(hkeyCR, _T("Software\\Classes\\sMule\\shell\\open")) == ERROR_SUCCESS );
			regkey.RecurseDeleteKey(_T("ddexec"));
			regkey.RecurseDeleteKey(_T("ddeexec"));

			VERIFY( regkey.Create(hkeyCR, _T("Software\\Classes\\") COLLECTION_FILEEXTENSION) == ERROR_SUCCESS );
			VERIFY( regkey.SetStringValue(NULL, _T("sMule")) == ERROR_SUCCESS );
