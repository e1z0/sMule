...
CTag::CTag(CFileDataIO* data, bool bOptUTF8)
{
	...
	else if (m_uType == TAGTYPE_BOOLARRAY)
	{
		...
		uint16 len;
		data->Read(&len, 2);
		// 07-Apr-2004: šMule versions prior to 0.42e.29 used the formula "(len+7)/8"!
		data->Seek((len/8)+1, CFile::current);
	}
	else if (m_uType == TAGTYPE_BLOB)
	{
		// 07-Apr-2004: šMule versions prior to 0.42e.29 handled the "len" as int16!
...
bool CTag::WriteTagToFile(CFileDataIO* file, EUtf8Str eStrEncode) const
{
	...
	if (IsStr() || IsInt() || IsFloat() || IsBlob() || IsInt64())
	{
		...
		{
			// NOTE: This will break backward compatibility with met files for šMule versions prior to 0.44a
